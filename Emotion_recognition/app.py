...
if __name__ == "__main__":
    ...
    # create header label
    header = Label(root, text="facial emotion recognition", font=("Arial", 24))
    header.pack(pady=20)

    # create image panel
    image = Image.new('RGB', (300,300), (300, 300, 300))
    image = ImageTk.PhotoImage(image)
    panel = Label(root, image=image)
    panel.pack(pady=20)

    # create start button
    btn_start = Button(root, text="Start Detection", font=("Arial", 14), command=start_detection)
    btn_start.pack(pady=10)

    # create stop button
    # btn_stop = Button(root, text="Stop Detection", font=("Arial", 14), command=stop_detection, state=DISABLED)
